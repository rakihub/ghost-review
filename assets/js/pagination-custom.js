// The custom pagination is used for tag/author page

export default function customPagination(isInfinite = true, done, isMasonry = false) {
    const mainElement = document.querySelector('.gh-main:has(.custom-loadmore)');
    if (!mainElement) return;

    const feedElement = mainElement.querySelector('.gh-feed');
    if (!feedElement) return;

    const buttonElement = document.querySelector('.custom-loadmore');
    ...
    const loadNextPage = async function () {
        ...
        try {
            ...
            const postElements = doc.querySelectorAll('.gh-feed:not(.gh-featured):not(.gh-related):not(.category-feed) > article');
            ...
            const loadButton = doc.querySelector('template');

            if (loadButton && resNextElement.href) {
